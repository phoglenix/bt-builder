...
void print_logo(glam2_aln *aln, data *d, int imotif) {
  ...
  printf("In print_logo in glam2_output.c alph size is %d\n", asize);
  ALPH_T alphabet = asize == DNA_ASIZE ? DNA_ALPH : asize == PROTEIN_ASIZE ? PROTEIN_ALPH : INVALID_ALPH;
  int both_strands = (alphabet == DNA_ALPH && d->a.two_strands);

  /* convert alignment to motif struct */
  MOTIF_T motif;                                                                              // GLEN: this is where motif is created
  ...
  for (i = 0; i < width; ++i) {
    for (j = 0; j < asize; ++j) {
      cell_value = aln->cols[i].emission_counts[j]/((double) aln->cols[i].match_count);
      if (aln->cols[i].match_count == 0) cell_value = 0;                                             // GLEN stopped -nan values when match count is 0
      // printf("In print_logo in glam2_output.c setting matrix cell (%d,%d) to %f/%f\n", (double)(aln->cols[i].emission_counts[j]), (double)(aln->cols[i].match_count));
      set_matrix_cell(i, j, 
