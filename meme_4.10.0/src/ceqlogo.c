...
typedef struct {
  ...
  int alph_size; // alphabet size ADDED BY GLEN
  double logo_height;
  double logo_width;
...
static void LP_add_line(LP_PARAMS_T* params, MATRIX_T* mat, int snum, int ltrim,
  ...
  if(alph_size(alphabet, ALPH_SIZE) > mat->num_cols)                // GLEN: mat->num_cols here is the alphabet size (I think)
    E_system("Alphabet size and matrix dimensions do not match!\n");
  
  params->alph_size = mat->num_cols;                                                //ADDED BY GLEN
  // printf("In LP_add_line in ceqlogo setting alph_size to %d\n", params->alph_size); //ADDED BY GLEN
...
*/
static void LS_calc(ARRAY_T* items, ALPH_T alph, int asize, size_t n, bool ssc, LSTACK_T* lstack) {
  int col, temp;
  //int asize;
  double sum, stack_height;
  //asize = alph_size(alph, ALPH_SIZE);
  // printf("In LS_calc in ceqlogo.c alph size is %d\n", asize);
  // sanity check
  ...
  if ((sum > 1.0 ? sum - 1.0 : 1.0 - sum) > 0.1) {
    printf("In LS_calc in ceqlogo: Warning: motif probabilities do not sum to 1. sum: %f\n", sum);
    // E_system("Motif probabilities do not sum to 1.");
  }
  // Over all letters of the alphabet
  stack_height = LS_calc_R(items, asize, n, ssc);
  // printf("In LS_calc in ceqlogo.c -----------\n");
  for (col = 0; col < asize; col++) {
    if (alph == INVALID_ALPH) {
      // no info about alphabet here so just cast int to char (and avoid bad chars hopefully) - - - GLEN
      temp = col + '0'; // avoid non-visible characters at the start
      if (temp >= '<') temp++; // avoid chars with special meaning
      if (temp >= '>') temp++;
      if (temp >= '?') temp++;
      if (temp >= '[') temp++;
      if (temp >= '\\') temp++;
      if (temp >= ']') temp++;
      if (temp >= '{') temp+=5; // avoid { at 123 through to DEL character at 127
      if (temp >= 256) temp = '?';
      // printf("In LS_calc in ceqlogo.c about to set char as %d\n", temp);
      // printf("In LS_calc in ceqlogo.c about to set char as %c\n", temp);
      // printf("In LS_calc in ceqlogo.c about to set char as %c\n", (char)temp);
      lstack[col].letter = (char)temp;                                                         // GLEN: added all this
    } else {
      lstack[col].letter = alph_char(alph, col);
    }
    // printf("In LS_calc in ceqlogo.c got past lstack[col].letter\n");
    lstack[col].height = get_array_item(col, items) * stack_height;
    // printf("In LS_calc in ceqlogo.c height was set to %f in col %d stack height %f\n", lstack[col].height, col, stack_height);
  }
...
static char* EPS_data_token(LP_PARAMS_T* params) {
  ...
  if (params->alphabet == INVALID_ALPH) asize = params->alph_size;
  ...
  for (lnr = 0; lnr < params->l_num; lnr++) {
    ...
    for (pos = 0; pos < num_rows; pos++) {
      ...
      LS_calc(get_matrix_row(pos, line->mat), params->alphabet, params->alph_size, line->snum, params->ssc, lstack); // GLEN ADDED ALPH SIZE HERE
...
static RBTREE_T* EPS_tokens(LP_PARAMS_T* params) {
  ...
  if (params->alphabet == DNA_ALPH) {
    ...
    rbtree_put(tokens, "$BARBITS", "4.3");
    rbtree_put(tokens, "$LOGOTYPE", "UNKNOWN");
    rbtree_put(tokens, "$COLORDICT", EPS_AAcolordict());
  }
...
*/
void CL_create2(MOTIF_T *motif1, char* label1, MOTIF_T *motif2, char* label2, // GLEN NOTE: motif1 is our motif, motif2 is null
    bool errbars, bool ssc, double height, double width, int shift, // motif has .freqs matrix with .num_cols field that is alph size
